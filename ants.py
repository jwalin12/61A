...
class Bee(Insect):
    ...
    def action(self, colony, forward=True):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        counter=0
        while place!=hive:
            if len(place.bees ) and self.min_range<=counter<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            bees_in_place=[bee for bee in place.bees]
                ...
                if place.ant!=None:
                    if isinstance(place.ant,BodyguardAnt):
                        if place.ant.contained_ant!=None:
                            if place.ant.contained_ant not in self.ants_behind:
                                self.ants_behind.append(place.ant.contained_ant)
                                place.ant.contained_ant.damage *=2
                    if isinstance(place.ant,TankAnt):
                        if place.ant not in self.ants_behind:
                            self.ants_behind.append(place.ant)
                            place.ant.damage*=2
                        if place.ant.contained_ant!=None and place.ant.contained_ant not in self.ants_behind:
                            self.ants_behind.append(place.ant.contained_ant)
                            place.ant.contained_ant.damage*=2
                    else:
                        if place.ant not in self.ants_behind:
...
def apply_effect(effect, bee, duration):
    ...
    newaction=effect(old_action,bee)
    def action_counter(colony):
        nonlocal duration
        if duration==0:
...
class ScaryThrower(ThrowerAnt):
    ...
    # END Problem EC
        ...
        # BEGIN Problem EC
    def throw_at(self, target):
        if target and target.been_scared==False:
